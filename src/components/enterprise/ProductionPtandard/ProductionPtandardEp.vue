    ...
    <div>
      ...
      <div class="header">
        <div class="mui-input-row mui-search mui-active">
          <input type="search" class="mui-input-clear" placeholder="请输入产品名称" data-input-clear="1"
                 v-model="productName"  v-on:change="getProductionStandardList()" data-input-search="1">
        ...
        <ul v-if="data.length">
          <li v-for="row in data" :key="row.id" @click="standardDetails(row.id)">{{row.productName}}<span class="mui-icon mui-icon-arrowright" style="margin-left: 8rem;margin-top: 1.6rem"></span></li>
        </ul>
    ...
    export default {
      ...
      methods:{
        standardDetails(selectedId){
          this.$router.push({path:'/StandardDetailsEp',query:{
